 ...
 *
 * $Id: MemoryBudget.java,v 1.1.2.4 2007/03/01 23:15:20 ckaestne Exp $
 */
 ...
 */
public class MemoryBudget extends LogBufferBudget {
	 ...
	 * behave predictably.
	 * 
	 * _32 values are the same on Windows and Solaris. _64 values are from
	 * 1.5.0_05 on Solaris. _14 values are from 1.4.2 on Windows and Solaris.
	 * _15 values are from 1.5.0_05 on Solaris and Windows.
	 * 
	 * Specifically:
	 * 
	 * java.vm.version=1.5.0_05_b05 os.name=SunOS java.vm.version=1.4.2_05_b04
	 * os.name=SunOS java.vm.version=1.5.0_04_b05, os.name=Windows XP
	 * java.vm.version=1.4.2_06-b03, os.name=Windows XP
	 * 
	 * The integer following the // below is the Sizeof argument used to compute
	 * the value.
	 */
	...
	/*
	 * Txn memory is the size for the Txn + a hashmap entry overhead for being
...
//				throw RE;
	...
	MemoryBudget(EnvironmentImpl envImpl, DbConfigManager configManager)
		 ...
		 * once per environment. The logic to do the calculations is left in the
		 * respective node classes so it can be done properly in the domain of
		 * those objects.
		 */
	...
	private static boolean isJava5VM() {
		try {
			return Class.forName("com.sleepycat.je.latch.Java5LatchImpl") != null;
	...
	protected void reset(DbConfigManager configManager) throws DatabaseException {
		super.reset(configManager);
		
		long newCriticalThreshold = (maxMemory * envImpl.getConfigManager()
				.getInt(EnvironmentParams.EVICTOR_CRITICAL_PERCENTAGE)) / 100;

		long newTrackerBudget = (maxMemory * envImpl.getConfigManager()
				.getInt(EnvironmentParams.CLEANER_DETAIL_MAX_MEMORY_PERCENTAGE)) / 100;
		...
		cacheBudget = maxMemory - logBufferBudget;
	...
	public long calcTreeCacheUsage() throws DatabaseException {
		...
		totalSize=hook_getSize(inList);
		return totalSize;
	}

	private long hook_getSize(INList inList) throws DatabaseException {
		long totalSize = 0;
	 ...
	 * 
	 * @param increment
	 *            note that increment may be negative.
	 */
	...
	public static int byteArraySize(int arrayLen) {
		...
		/*
		 * BYTE_ARRAY_OVERHEAD accounts for 4 bytes of data. Data larger than 4
		 * bytes is allocated in 8 byte increments.
		 */
